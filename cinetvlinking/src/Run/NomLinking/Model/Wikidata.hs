...
import Data.Text.ICU.Normalize (normalize, NormalizationMode(NFD))
...
instance GetWikidataPersonsCount (RIO env) where
  getCountsByFullname name = do
    ...
    return $ listToMaybe results >>= Map.lookup "count"
                                 >>= getLiteral
                                 >>= convertToNumber
  ...
  WHERE
  {
    ...
    $functionQueries
        ...
        |]
      functionQueries = Text.intercalate " UNION "
                      $ fmap functionToQuery allFunctions
...
instance GetWikidataPersonMatch (RIO env) where
  getMatchByNameAndRoles personName personFunctions = do
    ...
    return $ getIfSoleResult results
         >>= Map.lookup "item"
         >>= getIRI
    ...
    where
      ...
      WHERE
      {
        ...
        where
          functionQueries = Text.intercalate " UNION "
                          $ fmap functionToQuery personFunctions
          functionToQuery f = "{ ?item wdt:P106 wd:" <> f <> " }"
