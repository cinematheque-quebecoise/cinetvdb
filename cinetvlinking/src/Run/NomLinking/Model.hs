...
module Run.NomLinking.Model
  ( GetWikidataPersonsCount(..)
  , GetPersonRoles(..)
  , GetUsersByName(..)
  , getPersonFeatures
  , getNumResultsForPersonName
  , selectNomLienWikidataIds
  )
where
...
getPersonRoles' :: (MonadIO m) => Pool SqlBackend -> Int64 -> m [Text]
getPersonRoles' pool personId = do
  ...
  filmoGenericResults <- liftIO $ flip runSqlPersistMPool pool $ do
    select $ distinct $ from $ \(filmo, filmoGenerique, nom, fonction) -> do
      where_
        (   nom
        ^.  NomId
        ==. valkey personId
  ...
  filmoRealisationResults <- liftIO $ flip runSqlPersistMPool pool $ do
    select $ distinct $ from $ \(filmo, filmoRealisation, nom) -> do
      where_
        (   nom
        ^.  NomId
        ==. valkey personId
  ...
  return
    $  fmap toFunctionId filmoGenericResults
    <> if length filmoRealisationResults > 0 then ["realisation"] else []

    -- Nothing -> return []

toFunctionId :: (Entity Fonction) -> Text
toFunctionId fonctionEntity =
  Text.pack $ show $ fromSqlKey $ entityKey fonctionEntity
...
getPersonFeatures :: (MonadIO m) => Pool SqlBackend -> m [PersonFeatures]
...
selectNomLienWikidataIds :: (MonadIO m) => Pool SqlBackend -> m [Key Nom]
selectNomLienWikidataIds pool = do
...
creditsToPersonFeatures credits =
  (catMaybes . map personCreditsToFeatures . groupByKey
      (\(_, nomEntity, _) -> entityKey nomEntity)
    )
    credits

personCreditsToFeatures
  :: [(Entity Filmo, Entity Nom, Entity Fonction)] -> Maybe PersonFeatures
personCreditsToFeatures personCredits = fmap toPersonFeatures
 ...
 where
  ...
  toPersonFeatures (_, nomEntity, _) =
    ...
    in  PersonFeatures
          ...
          lastname
          ""
          (Text.intercalate " ; " $ map personMovieRoleToText $ groupByKey
            (\(fe, _, _) -> entityKey fe)
            personCredits
          )
...
personMovieRoleToText movieCredits =
  let
    movieText = fromMaybe "" $ do
      ...
      let movieYearText =
            "(" <> (fromMaybe "" $ fmap (Text.pack . show) $ movieYear) <> ")"
      return $ Text.intercalate " " [movieTitle, movieYearText]
    movieFunctions = Text.intercalate " ; " $ List.nub $ map
      (\(_, _, fonctionEntity) -> fonctionTerme $ entityVal fonctionEntity)
      movieCredits
  in
    Text.intercalate " - " [movieText, movieFunctions]

getNumResultsForPersonName
  :: (MonadIO m) => Pool SqlBackend -> Text -> Text -> m Int
